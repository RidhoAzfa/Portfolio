...
function mockResponse(message: string): string {
  ...
  if (query.includes("celeste") || query.includes("workstation") || query.includes("inference") || query.includes("oom") || query.includes("diffusion") || query.includes("flux")) {
    return "⚡ **Celeste AI** is a state-of-the-art hybrid local/cloud Generative AI workstation.\n\nKey technical facets include:\n- **Decoupled Architecture**: React 19 SPA + Express API Proxy (Port 3001) + FastAPI GPU Engine (Port 5000).\n- **GPU OOM Protection**: Forks intensive tasks to an isolated `local_worker.py` subprocess.\n- **VRAM Optimizations**: Peak VRAM under 4.5 GB via sequential CPU offloads, VAE slicing, and tiling.\n- **IndexedDB Vault**: Client-side storage (CelesteDB) bypasses LocalStorage's 5MB quota to store generated media.";
  }
